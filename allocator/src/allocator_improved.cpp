...
void getLiveHosts(string filename, int mode = 0){

    set<string> pragma_nodes;
    // create a set of hosts which are present in pragma file
    ifstream pragma_file(homedir+"/.eagle/pragma_hosts.txt");
    // create set of hosts only if pragma file is present
    if(pragma_file.is_open())
    {
        string hostname = "";
        while(pragma_file >> hostname)
        {
            pragma_nodes.insert(hostname);
        }
    }
	...
	while(hostfile >> hostname){
        ...
        // If mode is set, then pickup hosts only if
        //they are present in pragma hosts file
        if(mode == 1)
        {
            if(pragma_nodes.find(hostname) != pragma_nodes.end())
            {
                cout << hostname << " ";
...
int main(int argc, char **argv){
	if(argc<4){
		cout<<"Usage: <executable> <number of hosts> <ppn> <outfile> <pragma mode (optional) >"<<endl;
		exit(0);
	}
	homedir = getenv("HOME");
    FILE *logfile;
    ...
    // if pragma mode is enabled then pick hosts only from pragma file.
    // Pragma file is at ~/.eagle/pragma_hosts.txt
    if(argc > 4)
        mode = atoi(argv[4]);
	// Get list of hosts
	getLiveHosts(homedir+"/.eagle/livehosts.txt", mode);
	...
	FILE *hosts;
	char hosts_out[255];
	snprintf(hosts_out,strlen(argv[3])+20,"%s_%d_%d",argv[3],required_cores, ppn);
	hosts = fopen(hosts_out,"w");
	if(DEBUG){
		for(auto it = G.begin(); it!=G.end();it++){
